...
def mkq(q, cat, label):
    opts = q["options"][:]
    random.shuffle(opts)
    return {"id": q["id"], "cat": cat, "label": label, "text": q["text"], "options": opts}
...
def main():
    ...
    for cat, areas in CATEGORIES.items():
        ...
        qs = [q for q in qs if any(o["correct"] for o in q["options"])]  # must have a key
        pool[cat] = qs
        print(f"{cat}: {len(qs)} usable questions", file=sys.stderr)
